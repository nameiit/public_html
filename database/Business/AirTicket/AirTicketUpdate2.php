...
$query = "SELECT airticket_tour_id, tc_id FROM Transactions WHERE transaction_id = $transactionId_old";
$result = $conn->query($query);
if ($result->num_rows > 0) {
    $temp = $result->fetch_assoc();
    $airticket_tour_id_old = $temp['airticket_tour_id'];
    $tc_id = $temp['tc_id'];
}
...
$conn->query($query);
$query = "DELETE FROM AirticketNumber WHERE airticket_tour_id = $airticket_tour_id_old";
$conn->query($query);
$query = "DELETE FROM AirSchedule WHERE airticket_tour_id = $airticket_tour_id_old";
$conn->query($query);
$query = "DELETE FROM AirScheduleIntegrated WHERE airticket_tour_id = $airticket_tour_id_old";
$conn->query($query);
$query = "SELECT IFNULL(mp_id, 'KK') AS mp_id FROM AirticketTour WHERE airticket_tour_id = $airticket_tour_id_old";
$result = $conn->query($query);
$mp_id = $result->fetch_assoc()['mp_id'];
$query = "DELETE FROM AirticketTour WHERE airticket_tour_id = $airticket_tour_id_old";
$conn->query($query);
if ($mp_id != 'KK') {
...
$sql = "UPDATE AirticketTour SET airticket_tour_id = $airticket_tour_id_old WHERE airticket_tour_id = $airticket_tour_id";
$conn->query($sql);
$airticket_tour_id = $airticket_tour_id_old;
...
$sql = "UPDATE Transactions
        SET transaction_id = $transactionId_old, 
        tc_id = $transactionId_old
